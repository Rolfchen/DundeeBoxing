...
static NSString *kDirectory = @"folder";
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    NSString *directory = [comboDetail objectForKey:kDirectory];
    NSDictionary *moveData = [moves objectAtIndex:indexPath.row];
    if ([moveData objectForKey:videoName]!= nil) {        
        NSString *videoURLString = [[NSBundle mainBundle] pathForResource:[moveData objectForKey:videoName] ofType:[moveData objectForKey:videoType] inDirectory:directory];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    cell.imageView.image = [UIImage imageNamed:@"VideoListIcon.png"];
...
- (void)viewDidLoad
{
    ...
    comboBreakDownTable.rowHeight = 50;
    ...
    [videoButton setBackgroundImage:[self.player thumbnailImageAtTime:1 timeOption:MPMovieTimeOptionNearestKeyFrame] forState:UIControlStateNormal];
